...
Review approved Task 10 (Gray/Hearst/Nexstar consent letters) "with fixes," two content corrections applied identically across all three letters as a second commit:

- **Timestamp deep-link claim softened.** The letters originally claimed every published quote is "deep-linked back to the original recording... at the cited timestamp." The code only appends a timestamp (`&t=Ns`) for YouTube URLs (`insert_quotes.py`'s `with_timestamp`, `web/lib/sourceLink.ts`); station-site content gets a bare page link with no timestamp. Reworded to "linked back to the original recording on the publisher's own page or player (with a timestamp where the platform supports it)" in the opening paragraph, and "a link back to your page/player" (dropping "at the cited timestamp") in the "do NOT do" bullet, so the claim no longer overstates what station-site content actually gets.
- **Monitoring-vs-fetch distinction made explicit, then corrected on re-review.** Since these letters go to legal teams who will read the "we honor robots.txt and poll only public syndication endpoints" bullet closely, the first pass drew a line at "individual items are fetched for processing only after a human approves them" — but that overstated how much of the pipeline is human-gated. The actual split, per `src/discovery/classify.py`'s `classify_item`: (1) ongoing monitoring polls only public RSS/syndication endpoints, honoring robots.txt and per-domain politeness delays; (2) classification itself can trigger an automated, unattended `peek_fetcher` GET of the specific item's own page text (a second LLM pass fired when the first pass's confidence lands in the mid band) to help judge relevance — this happens before any human sees the item; (3) only full content extraction and transcription of an item is gated on human approval. The bullet was corrected to name all three stages honestly: "ongoing monitoring polls only public syndication endpoints, honors robots.txt, and applies per-domain politeness delays. Judging whether a specific item is relevant may include a brief automated read of that item's own page text; full content extraction and transcription of an item happens only after a human approves it."

The files in `docs/outreach/` are authoritative over Task 10's verbatim letter text quoted earlier in this plan; where they differ, the shipped letters reflect this amendment.

### Final review (2026-08-05)

Final whole-branch review approved with two Minor fixes, applied as a follow-up commit:

- **WTMJ fixture label corrected 3→2.** `tests/fixtures/discovery_eval_real.jsonl` line 1 (the "Milwaukee County Exec David Crowley Launches Bid For Governor" row) was labeled `expected_tier: 3`, treating it as an announcement speech. The row's own description says "FULL INTERVIEW: … joined WTMJ's Steve Scaffidi on Point Taken" — a radio news-org interview — which the questioner-independence ladder places at tier 2, not tier 3. Corrected.
- **Runbook eval-upkeep gains a labeling instruction.** `docs/runbooks/source-discovery.md`'s "Eval upkeep" section didn't say that rows harvested by `harvest_discovery_verdicts.py` arrive without `expected_tier`. Added a sentence noting these rows must be hand-labeled per the questioner-independence ladder before `tier_accuracy` is meaningful, since unlabeled rows silently drop out of the metric instead of counting as wrong.
