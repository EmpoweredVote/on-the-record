...
- **Monitoring-vs-fetch distinction made explicit.** Since these letters go to legal teams who will read the "we honor robots.txt and poll only public syndication endpoints" bullet closely, it was clarified that ongoing polling (monitoring for new items) is separate from fetching an individual item for processing, and the latter happens only after a human approves it — the bullet now reads "our monitoring polls only public syndication endpoints, honors robots.txt, and applies per-domain politeness delays; individual items are fetched for processing only after a human approves them," preempting the question of whether the automated polling itself constitutes unconsented access to GLM/station content.
