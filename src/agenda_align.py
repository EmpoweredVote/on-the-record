...
_ALIGN_SYSTEM = (
    ...
    "ONLY when it is decided on the record: the chair announces the result "
    "(motion carries / does not carry) or a dispositive motion (be adopted, "
    "be tabled, be postponed) is voted to completion. When several motions "
    "occur within one item, the outcome is the disposition of the LAST "
    "dispositive motion (a resolution moved for adoption but then tabled is "
    "continued, not passed). A first reading referred to a later meeting "
    "without a final vote has outcome null. For outcome_evidence_segment "
    "cite the segment whose OWN text carries the outcome wording — the "
    "announcement (carries, does not carry) or the dispositive motion's "
    "action word (adopted, tabled, postponed) — never a segment that is "
    "only roll-call yes/no responses; if no segment carries such wording, "
    "report outcome null. Do NOT infer or invent outcomes. "
    "Procedural stretches "
