...
1. **Meeting outline ("items").** A meeting renders as the list of substantive things it deliberated — each item is a summarizer **section** (discussion / public comment / consent agenda / vote) with its title/bill name, time range, outcome, jump link, and **topic label(s)**. Topics are labels *on* items, not free-floating chips. (Chosen over chips in the visual brainstorm.)
